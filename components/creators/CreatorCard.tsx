...
export default function CreatorCard({ member, avgRating, totalReviews = 0 }: CreatorCardProps) {
  ...
  const href = member.crew_slug
    ? `/smart-creators/${member.crew_slug}`
    : `/smart-creators/${member.id}`;
        ...
        <div className="relative w-full aspect-square bg-neutral-900 overflow-hidden">
          {member.avatar_url ? (
            <Image
              ...
              alt={member.display_name || 'Creator'}
              fill
