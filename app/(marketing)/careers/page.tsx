      ...
      <div className="container mx-auto px-6">
        ...
        <div className="max-w-4xl mb-24">
          ...
          <h1 className="text-5xl md:text-8xl font-display font-medium text-white tracking-tighter uppercase mb-8 leading-[0.9]">
            Join The <br />
            <span className="text-neutral-600">Vision</span>
          </h1>
        ...
        {/* Culture / Benefits */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-32 border-y border-white/10 py-12">
          <div>
            <h3 className="text-sm font-bold uppercase tracking-widest text-white mb-4">
              R&amp;D Access
            </h3>
            <p className="text-sm text-neutral-500 font-mono">
              Full access to our equipment, studio spaces, and tools for personal creative and R&amp;D projects.
            </p>
          </div>
          <div>
            <h3 className="text-sm font-bold uppercase tracking-widest text-white mb-4">
        ...
        {/* Open Positions */}
        <div className="mb-20">
          <h2 className="text-2xl font-display font-medium uppercase mb-12 text-white">
            Open Positions
          </h2>
          ...
          {published.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-6">
              {published.map(job => (
                <div
                  key={job.id}
                  className="group relative bg-[#0A0A0B] border border-white/5 p-8 md:p-10 hover:border-white/20 transition-all cursor-pointer"
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                      <h3 className="text-xl font-display font-bold uppercase text-white mb-2 group-hover:underline decoration-1 underline-offset-4">
                        {job.title}
                      </h3>
                      ...
                      {job.requirements && job.requirements.length > 0 && (
                        <ul className="mt-4 space-y-1">
                          {job.requirements.map((req, i) => (
                            <li
                              key={i}
                              className="text-xs font-mono text-neutral-500 flex items-start gap-2"
                            >
                              <span className="mt-0.5 text-[#D4A843]">—</span>
                              {req}
                            </li>
                          ))}
                        </ul>
        ...
        {/* General Application */}
        <div className="bg-[#080808] p-12 text-center border border-white/5">
          ...
          <a
            ...
            className="text-lg font-display text-[#D4A843] underline decoration-1 underline-offset-4 hover:text-[#D4A843]/70 transition-colors"
          >
            {STUDIO.careersEmail}
          </a>
        </div>
