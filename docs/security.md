...
**Build Status:** Passing (zero errors after all fixes)

A full re-audit was performed across all 7 security areas. Three new issues were found and fixed. Two lower-priority items remain pending.

### Fixed: Crew Requests Admin Auth Bypass (Critical)

**What was wrong:**
Both `/api/admin/crew-requests` (GET) and `/api/admin/crew-requests/[id]` (PATCH) had a broken auth check. The code did:
...
`requireAdmin()` returns an `AuthResult` object — which is always truthy in JavaScript. The `if (!admin)` check never blocked anyone. Any logged-in user could read all crew requests and update their status.

**How it was fixed:**
Changed both routes to use the same `requireAdmin()` pattern every other admin route uses:
...
**Files changed:**
- `app/api/admin/crew-requests/route.ts`
- `app/api/admin/crew-requests/[id]/route.ts`

---

### Fixed: Database Filter Injection in 3 Locations (High)

**What was wrong:**
Three places put user input directly into Supabase `.ilike()` or `.or()` filter strings without escaping special characters (`%`, `_`, `\`). These characters have meaning in database pattern matching. Crafted input could change what the query matches.

**Where it was happening:**
1. `lib/search/supabase.ts` — search queries went into `.or()` filters across 6 tables
2. `lib/ubunye/tools.ts` — the AI crew search tool passed `location` straight into `.ilike()`
3. `app/(admin)/admin/rentals/import/page.tsx` — PDF-extracted item titles went into `.or()` unescaped

**How it was fixed:**
Created a shared `escapeIlike()` helper in `lib/search/types.ts` that escapes `\`, `%`, and `_` before they reach the database. Applied it in all three locations.
...
**Files changed:**
- `lib/search/types.ts` (new helper)
- `lib/search/supabase.ts`
- `lib/ubunye/tools.ts`
- `app/(admin)/admin/rentals/import/page.tsx`

---

### Fixed: Missing `/api/orders/[id]` Route (Medium)

**What was wrong:**
The `/api/orders/[id]` endpoint didn't exist. Any code trying to fetch or update a single order got a 404.

**How it was fixed:**
Created the route with proper auth and ownership checks:
- **GET** — returns order with listing details, only if you're the buyer or seller
- **PUT** — buyers can only cancel pending orders, sellers can mark shipped/delivered/completed, payment statuses (paid/refunded) are system-only via PayFast ITN
...
### Pending: In-Memory Rate Limiting (Medium)

Rate limiting uses a JavaScript `Map` in server memory. On Vercel, each serverless instance has its own memory, so rate limit counters aren't shared across instances. A determined attacker could bypass limits by hitting different instances.

**Current risk:** Low at current traffic levels.
**Future fix:** Replace the `Map` in `lib/auth/index.ts` with Vercel KV or Upstash Redis (both free tier). All instances would then check the same counter.

---

### Pending: UUID String Interpolation in `.or()` Filters (Low)

Two places put `user.id` (a UUID from the auth session) directly into `.or()` filter strings:
- `app/api/orders/route.ts:23`
- `app/(platform)/dashboard/page.tsx:38`

UUIDs can't contain injection characters, so this is safe in practice but doesn't follow the best practice of parameterised queries.

**Future fix:** Refactor to use chained `.eq()` filters instead of string interpolation.

---

### What Passed (No Issues)

- **XSS:** Zero `dangerouslySetInnerHTML`. All user content auto-escaped by React. Social links validated to `http:`/`https:` only.
- **Secrets:** No API keys in source code. `.env*` in `.gitignore`.
- **API Auth:** 35/35 routes have correct auth (including the new `orders/[id]`). All 16 admin routes use `requireAdmin()`.
- **User Data Isolation:** Every endpoint filters by `user_id` — users can't access other users' data.
- **Payment Webhooks:** PayFast ITN validates signatures, source IP, and reconciles amounts.
- **CSRF:** Proxy checks `Origin` header on API mutations.
- **Rate Limiting:** Applied on chat (10/min), bookings (5/min), subscriptions (3/min), contact (3/min), cloudinary (20/min).
...
*Updated: 2026-03-19 — 3 new issues found and fixed, 2 lower-priority items pending.*
